...
class RobotInterface(object):
    ...
    def get_object_xpos_by_name(self, obj_name, obj_type_str):
        """Gets the position of a named object."""
        if obj_type_str == 'OBJ_BODY':
            return self.data.body(obj_name).xpos
        elif obj_type_str == 'OBJ_GEOM':
            return self.data.geom(obj_name).xpos
        else:
            raise ValueError(f"Unsupported object type string: {obj_type_str}")

    def get_obstacle_pos(self, obstacle_id):
        """Gets the position of a specific obstacle by its ID."""
        obstacle_name = f'obstacle_{obstacle_id}'
        return self.get_object_xpos_by_name(obstacle_name, 'OBJ_GEOM')

    def set_obstacle_pos(self, obstacle_id, pos):
        """Sets the position of a specific obstacle by its ID."""
        obstacle_name = f'obstacle_{obstacle_id}'
        geom_id = mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_GEOM, obstacle_name)
        if geom_id != -1:
            self.model.geom_pos[geom_id] = pos
        else:
            raise ValueError(f"Obstacle geom '{obstacle_name}' not found in model.")

    def check_obstacle_collisions(self, num_obstacles):
        """Checks for collisions between the robot and any obstacle."""
        robot_geom_names = {self.model.geom(i).name for i in range(self.model.ngeom) if self.model.geom_bodyid[i] > 0}
        obstacle_geom_names = {f'obstacle_{i}' for i in range(num_obstacles)}
        ...
        for i in range(self.data.ncon):
            ...
            is_robot_obstacle_collision = (geom1_name in robot_geom_names and geom2_name in obstacle_geom_names) or \
                                          (geom2_name in robot_geom_names and geom1_name in obstacle_geom_names)

            if is_robot_obstacle_collision:
