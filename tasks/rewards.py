...
def _calc_obstacle_avoidance_reward(self, num_obstacles):
    """
    Calculates a reward for avoiding obstacles.
    The reward is higher when the robot is further away from the nearest obstacle.
    """
    robot_pos = self._client.get_object_xpos_by_name(self._root_body_name, 'OBJ_BODY')
    min_dist = float('inf')

    # Find the distance to the nearest obstacle in the x-y plane
    for i in range(num_obstacles):
        obs_pos = self._client.get_obstacle_pos(i)
        dist = np.linalg.norm(robot_pos[:2] - obs_pos[:2])
        if dist < min_dist:
            min_dist = dist
    
    # Define a safe distance. If the robot is closer, it gets penalized.
    safe_distance = 1.5  # meters
    penalty = 0.0

    if min_dist < safe_distance:
        # Penalty increases as the robot gets closer to the obstacle
        penalty = 10.0 * (safe_distance - min_dist)
    
    # The reward is an exponential decay of the penalty
